...
import { products, categories, sportCategories } from '../data/products';
...
export default function ShopPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return filtered;
  }, [category, sportFilter, sortBy]);

  const categoryName = category
    ? categories.find(c => c.id === category)?.name || sportCategories.find(s => s.id === category)?.name
    : 'Все товары';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl text-[#C5C3C6] mt-2">
            {sportFilter ? sportFilter.toUpperCase() : categoryName}
          </h1>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-20">
            <Filter className="w-12 h-12 text-[#5C677D] mx-auto mb-4" />
            <h3 className="font-display text-xl text-[#C5C3C6]">Товары не найдены</h3>
            <p className="mt-2 text-sm text-[#5C677D]">Попробуйте изменить параметры фильтрации</p>
          </div>
        )}
      </div>
    </div>
